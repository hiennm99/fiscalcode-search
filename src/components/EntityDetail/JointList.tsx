...
import type { Joint } from "../../types/related.types";
import {entityApiService} from "../../services/entityApi.service.ts";
import {BORROWER_TYPES} from "../../constants";
import {Link} from "react-router";

interface JointListProps {
    entity: Entity;
}

const currentRole = BORROWER_TYPES['JOINT'];

export const JointList: React.FC<JointListProps> = ({ entity }) => {
    ...
    // Nếu entity hiện tại là joint (type 1), không hiển thị section này
    if (entity.borrower_type_id === currentRole) {
        return (
            <div className="bg-white rounded-lg shadow-sm p-12 text-center">
                <Users className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                <p className="text-gray-500">Current role is Joint Borrower</p>
            </div>
        );
    }
    ...
    if (joints.length === 0) {
        return (
            <div className="bg-white rounded-lg shadow-sm p-12 text-center">
                <Users className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                <p className="text-gray-500">No joint accounts found for this entity</p>
            </div>
        );
    }
